	...
	while(true)
		...
		for(unsigned int y=0;y<size[1];++y)
			...
			for(unsigned int x=0;x<size[0];++x,++ifPtr,++pdcPtr,++abPtr,sPtr+=3,++ofPtr,++nofPtr)
				...
				if((scanMask.empty()||scanMask[y*size[0]+x])&&newVal<Kinect::FrameSource::invalidDepth&&minD>=0.0f&&maxD<=0.0f)
					{
		...
		if(spatialFilter&&scanMask.empty())
			{
        ...
        if(spatialFilter&&!scanMask.empty())
            {
            // Separable smoothing: never mix selected pixels with walls outside.
            const int w=size[0],h=size[1];
            float* data=newOutputFrame.getData<float>();
            std::vector<float> temp(w*h);
            for(int pass=0;pass<2;++pass)
                {
                for(int y=0;y<h;++y) for(int x=0;x<w;++x)
                    {
                    int i=y*w+x; temp[i]=data[i];
                    if(!scanMask[i]) continue;
                    float sum=2*data[i]; int weight=2;
                    if(x>0&&scanMask[i-1]) {sum+=data[i-1];++weight;}
                    if(x+1<w&&scanMask[i+1]) {sum+=data[i+1];++weight;}
                    temp[i]=sum/weight;
                    }
                for(int y=0;y<h;++y) for(int x=0;x<w;++x)
                    {
                    int i=y*w+x;
                    if(!scanMask[i]) continue;
                    float sum=2*temp[i]; int weight=2;
                    if(y>0&&scanMask[i-w]) {sum+=temp[i-w];++weight;}
                    if(y+1<h&&scanMask[i+w]) {sum+=temp[i+w];++weight;}
                    data[i]=sum/weight;
                    }
                }
            }
...
FrameFilter::FrameFilter(const unsigned int sSize[2],unsigned int sNumAveragingSlots,const FrameFilter::PixelDepthCorrection* sPixelDepthCorrection,const PTransform& depthProjection,const Plane& basePlane,const std::vector<unsigned char>& mask)
	:scanMask(mask),pixelDepthCorrection(sPixelDepthCorrection),
	 averagingBuffer(0),
	 statBuffer(0),
