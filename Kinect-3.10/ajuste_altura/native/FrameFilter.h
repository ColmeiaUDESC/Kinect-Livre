...
class FrameFilter
	...
	public:
	FrameFilter(const unsigned int sSize[2],unsigned int sNumAveragingSlots,const PixelDepthCorrection* sPixelDepthCorrection,const PTransform& depthProjection,const Plane& basePlane,const std::vector<unsigned char>& mask=std::vector<unsigned char>()); // Creates a filter for frames of the given size and the given running average length
