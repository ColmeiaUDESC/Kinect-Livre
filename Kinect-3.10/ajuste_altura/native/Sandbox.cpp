...
Sandbox::DataItem::DataItem(void)
	:panelPreview(0),waterTableTime(0.0),
...
void Sandbox::rawDepthFrameDispatcher(const Kinect::FrameBuffer& frameBuffer)
	{
	if(cameraPreviewDepth&&!cameraPreviewPath.empty())
        cameraFrames.postNewValue(frameBuffer);
...
void Sandbox::receiveColorFrame(const Kinect::FrameBuffer& frameBuffer)
    {
    // Only hand off a reference here; never write files on the capture thread.
    cameraFrames.postNewValue(frameBuffer);
    }
...
void printUsage(void)
	...
	std::cout<<"  -cameraDepthPreview <PPM file>: raw depth used by the terrain, at 5 Hz"<<std::endl;
	std::cout<<"  -cameraPreview <PPM file>: RGB camera preview, up to 320x240 at 5 Hz"<<std::endl;
	std::cout<<"  -h"<<std::endl;
	...
	std::cout<<"  -crop <left %> <right %> <bottom %> <top %> (0..45 each)"<<std::endl;
...
Sandbox::Sandbox(int& argc,char**& argv)
	...
	{
	previewRate=5.0;
	nextCameraPreview=0.0;
	cameraPreviewDepth=false;
    scanViewAngle=0; scanViewSize=100;
	for(int i=0;i<4;++i) cropMargins[i]=0.0;
	...
	for(int i=1;i<argc;++i)
		...
		if(argv[i][0]=='-')
			...
			else if(strcasecmp(argv[i]+1,"cameraPreview")==0||strcasecmp(argv[i]+1,"cameraDepthPreview")==0)
                {
                if(i+1>=argc) throw std::runtime_error("-cameraPreview requires a file path");
                cameraPreviewDepth=strcasecmp(argv[i]+1,"cameraDepthPreview")==0;
                cameraPreviewPath=argv[++i];
                }
            else if(strcasecmp(argv[i]+1,"scanArea")==0)
                {
                if(i+1>=argc) throw std::runtime_error("-scanArea requires a file path");
                scanAreaPath=argv[++i];
                }
            else if(strcasecmp(argv[i]+1,"scanView")==0)
                {
                if(i+2>=argc) throw std::runtime_error("-scanView requires angle and size percent");
                scanViewAngle=atof(argv[++i]); scanViewSize=atof(argv[++i]);
                if(!std::isfinite(scanViewAngle)||!std::isfinite(scanViewSize)||scanViewSize<40||scanViewSize>150)
                    throw std::runtime_error("Invalid scan view size or angle");
                }
			else if(strcasecmp(argv[i]+1,"panelPreview")==0)
				{
				if(i+1>=argc) throw std::runtime_error("-panelPreview requires a file path");
				previewPath=argv[++i];
				}
			else if(strcasecmp(argv[i]+1,"previewRate")==0)
				{
				if(i+1>=argc) throw std::runtime_error("-previewRate requires a rate");
				char* end=0;
				const char* value=argv[++i];
				previewRate=strtod(value,&end);
				if(end==value||*end!='\0'||!(previewRate>=0.0&&previewRate<=10.0))
					throw std::runtime_error("-previewRate must be between 0 and 10");
				}
			else if(strcasecmp(argv[i]+1,"crop")==0)
				{
				if(i+4>=argc)
					throw std::runtime_error("-crop requires left right bottom top percentages");
				for(int j=0;j<4;++j)
					{
					char* end=0;
					const char* value=argv[++i];
					cropMargins[j]=strtod(value,&end);
					if(end==value||*end!='\0'||!(cropMargins[j]>=0.0&&cropMargins[j]<=45.0))
						throw std::runtime_error("-crop percentages must be numbers between 0 and 45");
					}
				}
    ...
    if(!scanAreaPath.empty())
        {
        ScanArea area; area.load(scanAreaPath);
        scanMask=area.mask(frameSize[0],frameSize[1]);
        PTransform::HVector planeCc(basePlane.getNormal());
        planeCc[3]=-basePlane.getOffset();
        PTransform::HVector planeDic(cameraIps.depthProjection.getMatrix().transposeMultiply(planeCc));
        if(std::abs(planeDic[2])<1e-12) throw std::runtime_error("Cannot intersect scan area with base plane");
        const int order[4]={0,1,3,2};
        for(int i=0;i<4;++i)
            {
            int j=order[i];
            Kinect::LensDistortion::Point pixel(area.p[j][0]*frameSize[0],(1.0-area.p[j][1])*frameSize[1]);
            pixel=cameraIps.depthLensDistortion.undistortPixel(pixel);
            const double depth=-(pixel[0]*planeDic[0]+pixel[1]*planeDic[1]+planeDic[3])/planeDic[2];
            basePlaneCorners[i]=cameraIps.depthProjection.transform(Point(pixel[0],pixel[1],depth));
            for(int k=0;k<3;++k) if(!std::isfinite(basePlaneCorners[i][k]))
                throw std::runtime_error("Invalid scan area projection onto base plane");
            }
        }
	...
	frameFilter=new FrameFilter(frameSize,numAveragingSlots,pixelDepthCorrection,cameraIps.depthProjection,basePlane,scanMask);
    ...
    if(!cameraPreviewPath.empty()&&!cameraPreviewDepth&&camera->getColorSpace()!=Kinect::FrameSource::RGB)
        {
        std::cerr<<"Camera preview unavailable: source does not provide RGB"<<std::endl;
        cameraPreviewPath.clear();
        }
    camera->startStreaming((cameraPreviewPath.empty()||cameraPreviewDepth)?0:Misc::createFunctionCall(this,&Sandbox::receiveColorFrame),
                           Misc::createFunctionCall(this,&Sandbox::rawDepthFrameDispatcher));
	
	/* Create the depth image renderer: */
	depthImageRenderer=new DepthImageRenderer(frameSize,scanMask);
    for(int i=0;i<4;++i) scanCorners[i]=basePlaneCorners[i];
...
void Sandbox::frame(void)
	{
    if(!cameraPreviewPath.empty()&&Vrui::getApplicationTime()>=nextCameraPreview)
        {
        nextCameraPreview=Vrui::getApplicationTime()+0.2;
        if(cameraFrames.lockNewValue())
            writeCameraPreview(cameraPreviewPath,cameraFrames.getLockedValue(),cameraPreviewDepth);
        }
...
void Sandbox::display(GLContextData& contextData) const
    ...
    if(!scanMask.empty()&&!(rs.fixProjectorView&&rs.projectorTransformValid))
        {
        // Fit the selected physical quadrilateral without perspective cropping.
        Vector normal=depthImageRenderer->getBasePlane().getNormal();
        Vector right=(scanCorners[1]-scanCorners[0])+(scanCorners[3]-scanCorners[2]);
        right-=normal*(right*normal); right.normalize();
        Vector up=normal^right;
        const double angle=scanViewAngle*3.14159265358979323846/180.0;
        Vector r=right*std::cos(angle)+up*std::sin(angle);
        Vector u=up*std::cos(angle)-right*std::sin(angle);
        Point center=Geometry::mid(Geometry::mid(scanCorners[0],scanCorners[1]),Geometry::mid(scanCorners[2],scanCorners[3]));
        double ex=0,ey=0;
        for(int i=0;i<4;++i)
            {
            Vector delta=scanCorners[i]-center;
            ex=std::max(ex,std::abs(delta*r)); ey=std::max(ey,std::abs(delta*u));
            }
        GLint viewport[4]; glGetIntegerv(GL_VIEWPORT,viewport);
        double aspect=double(viewport[2])/std::max(1,viewport[3]);
        double hh=std::max(ey,ex/aspect)*1.05*100.0/scanViewSize;
        if(hh<1e-6) hh=1;
        double hw=hh*aspect;
        PTransform fitted=PTransform::identity;
        for(int j=0;j<3;++j)
            {
            fitted.getMatrix()(0,j)=r[j]/hw;
            fitted.getMatrix()(1,j)=u[j]/hh;
            fitted.getMatrix()(2,j)=-normal[j]/10000.0;
            }
        for(int i=0;i<3;++i)
            {
            fitted.getMatrix()(i,3)=0;
            for(int j=0;j<3;++j) fitted.getMatrix()(i,3)-=fitted.getMatrix()(i,j)*center[j];
            }
        projection=fitted;
        projection*=Geometry::invert(ds.modelviewNavigational);
        }
	...
	/* Mask the final output without rescaling the calibrated image or affecting off-screen render passes. */
	if(cropMargins[0]>0.0||cropMargins[1]>0.0||cropMargins[2]>0.0||cropMargins[3]>0.0)
		{
		GLint viewport[4];
		glGetIntegerv(GL_VIEWPORT,viewport);
		int left=int(viewport[2]*cropMargins[0]/100.0+0.5);
		int right=int(viewport[2]*cropMargins[1]/100.0+0.5);
		int bottom=int(viewport[3]*cropMargins[2]/100.0+0.5);
		int top=int(viewport[3]*cropMargins[3]/100.0+0.5);
		...
		glClearDepth(1.0);
		const int rectangles[4][4]={
			{viewport[0],viewport[1],left,viewport[3]},
			{viewport[0]+viewport[2]-right,viewport[1],right,viewport[3]},
			{viewport[0],viewport[1],viewport[2],bottom},
			{viewport[0],viewport[1]+viewport[3]-top,viewport[2],top}};
		for(int i=0;i<4;++i)
			if(rectangles[i][2]>0&&rectangles[i][3]>0)
				{
				glScissor(rectangles[i][0],rectangles[i][1],rectangles[i][2],rectangles[i][3]);
	...
	if(!previewPath.empty())
		{
		if(!dataItem->panelPreview) dataItem->panelPreview=new PanelPreview;
		GLint viewport[4];
		glGetIntegerv(GL_VIEWPORT,viewport);
		dataItem->panelPreview->update(previewPath,Vrui::getApplicationTime(),viewport,previewRate);
		}
