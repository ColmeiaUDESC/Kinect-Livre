...
class Sandbox:public Vrui::Application,public GLObject
	...
	struct DataItem:public GLObject::DataItem
		...
		/* Elements: */
		public:
		PanelPreview* panelPreview; // Shared-render preview and FPS telemetry
	...
	Threads::TripleBuffer<Kinect::FrameBuffer> cameraFrames;
	std::string cameraPreviewPath;
	bool cameraPreviewDepth;
    std::string scanAreaPath;
    std::vector<unsigned char> scanMask;
    Point scanCorners[4];
    double scanViewAngle,scanViewSize;
	double nextCameraPreview;
	std::string previewPath;
	double previewRate;
	double cropMargins[4]; // Output crop percentages: left, right, bottom, top
	...
	void receiveColorFrame(const Kinect::FrameBuffer& frameBuffer);
