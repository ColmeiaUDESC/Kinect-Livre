...
DepthImageRenderer::DepthImageRenderer(const unsigned int sDepthImageSize[2],const std::vector<unsigned char>& mask)
	:depthImageVersion(0)
	{
    ...
    const unsigned int w=depthImageSize[0],h=depthImageSize[1];
    rowStarts.resize(h,0); rowCounts.resize(h,w*2);
    if(!mask.empty())
        for(unsigned int y=1;y<h;++y)
            {
            unsigned int first=w,last=0;
            for(unsigned int x=0;x<w;++x)
                if(mask[y*w+x]&&mask[(y-1)*w+x]) {first=std::min(first,x);last=x;}
            rowStarts[y]=first<w?first*2:0;
            rowCounts[y]=first<last?(last-first+1)*2:0;
            }
...
void DepthImageRenderer::renderSurfaceTemplate(GLContextData& contextData) const
	...
	for(unsigned int y=1;y<depthImageSize[1];++y,indexPtr+=depthImageSize[0]*2)
		glDrawElements(GL_QUAD_STRIP,rowCounts[y],GL_UNSIGNED_INT,indexPtr+rowStarts[y]);
