...
class DepthImageRenderer:public GLObject
	...
	/* Elements: */
	std::vector<unsigned int> rowStarts,rowCounts;
	...
	/* Constructors and destructors: */
	public:
	DepthImageRenderer(const unsigned int sDepthImageSize[2],const std::vector<unsigned char>& mask=std::vector<unsigned char>()); // Creates an elevation renderer for the given depth image size
